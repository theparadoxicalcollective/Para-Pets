...
- **Molten Blocks Mini-Game** (MoltenBlocksPage.tsx — completed):
  ...
  - **Lives & scoring**: 3 lives per run. Each top-out costs a life and resets the playfield (score & coins persist); 0 lives → game over. Scoring is `n × 3` points per row cleared (multi-row clears scale linearly). Every 35 points crossed grants 5 coins (`Math.floor(nextScore/35) - Math.floor(prevScore/35)` tiers, so a multi-tier crossing in one clear awards correctly).
