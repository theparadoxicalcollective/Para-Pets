...
- **Daily Quest System** (completed):
  - Three daily quests auto-seeded on first boot: "Feed Active Pet" (10×), "Gone Fishing" (5×), "Power Up Your Pet" (3×).
  ...
  - Progress hooks: feeding a pet increments `feed_pet`, catching a fish increments `catch_fish`, using a power-up on a pet increments `use_powerup`. All hooks are fire-and-forget (`.catch(() => {})`) so quest bugs never break core gameplay.
...
- **Molten Blocks Mini-Game** (MoltenBlocksPage.tsx — completed):
  ...
  - Hi-score persisted via `localStorage` only (no DB writes) so it Just Works on Railway with no migrations. `prevHi` is captured at start-of-run so the end overlay can show a "✦ NEW HIGH SCORE ✦" badge when this run beat the prior best.
  - **Lives & scoring**: 3 lives per run. Each top-out costs a life and resets the playfield (score & coins persist); 0 lives → game over. Scoring is `n × 10` points per row cleared (multi-row clears scale linearly). Every 100 points crossed grants 10 coins (`Math.floor(nextScore/100) - Math.floor(prevScore/100)` tiers, so a multi-tier crossing in one clear awards correctly). Multi-row bonuses on top: 2 rows +1, 3 rows +3, 4+ rows +5.
  - **Coin reward flow**: `coinsEarned` is tallied client-side during the run and submitted in a single batched POST to `/api/games/molten-blocks/reward` when the run ends. The end overlay shows score, lines, coins earned, best, and a live "Adding coins to your purse…" / "X coins added" status. After success, the `/api/auth/me` query is invalidated so the global coin counter updates immediately.
  - **Race-safety**: score / lives / coinsEarned are written to refs *synchronously* inside `lockAndClear` and `spawnNext` (in addition to React state setters), because a final line clear can immediately top out and call `finalizeRun` in the same tick — React state setters batch and would leave refs stale, causing the last clear's points/coins to be omitted from the persisted hi-score and reward POST.
  - **Backend endpoint** `POST /api/games/molten-blocks/reward` (`server/routes.ts` ~line 2104): `isAuthenticated`, calls `storage.addCoins`. Defense-in-depth (no DB migration): per-call cap 200 coins, per-user cooldown 4s between submissions (returns 429), per-user UTC-daily cap of 300 coins tracked in an in-memory Map (`moltenRewardState`). On server restart the daily counter resets, but the small cap bounds the worst case.
