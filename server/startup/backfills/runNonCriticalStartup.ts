...
import { backfillFisherBadges, backfillFishBookBadges, backfillMinorAcquisitionBadge, backfillAdvancedAcquisitionBadge, backfillCoinPurchaseEarnings, syncTotalCoinsEarnedFloor, seedBrawlerBadges, backfillBrawlerBadges } from "../../routes";
import fs from "fs";
import path from "path";

export async function runNonCriticalStartup(): Promise<void> {
  ...
  await runMigration("shop_items.pets_healed",
    () => db.execute(sql`ALTER TABLE shop_items ADD COLUMN IF NOT EXISTS pets_healed INTEGER`));
  await runMigration("user_inventory.xp_boost_until",
    () => db.execute(sql`ALTER TABLE user_inventory ADD COLUMN IF NOT EXISTS xp_boost_until TIMESTAMP`));
  await runMigration("user_inventory.xp_boost_pct",
    () => db.execute(sql`ALTER TABLE user_inventory ADD COLUMN IF NOT EXISTS xp_boost_pct INTEGER NOT NULL DEFAULT 0`));
  ...
  try {
    ...
    const defaultQuests: { key: string; title: string; desc: string; target: number; coinReward?: number; rewardItemId?: string; rewardItemQty?: number }[] = [
      ...
      { key: "play_molten_blocks",title: "Play Molten Blocks",    desc: "Play a Molten Blocks game",      target: 1,  coinReward: 100 },
      { key: "sell_fish",         title: "Sell Fish",             desc: "Sell 10 fish",                   target: 10, rewardItemId: "7b381092-3b76-4c91-99bc-5a5ba91f52ec" },
    ];
    // Update play_molten_blocks reward from PvP ticket to 100 coins
    await db.execute(sql`
      UPDATE daily_quests
      SET coin_reward = 100, reward_item_id = NULL, reward_item_quantity = 1
      WHERE quest_key = 'play_molten_blocks'
    `);
