...
export function registerClearingAdminRoutes(app:Express,{db,isAdmin}:{db:any;isAdmin:RequestHandler}){
 ...
 app.delete("/api/admin/clearing/drops/:id",isAdmin,async(req,res)=>{try{const id=req.params.id as string;const removed=await db.transaction(async(tx:any)=>{const f=await tx.execute(sql`SELECT world_id,shop_item_id FROM clearing_world_drops WHERE id=${id} FOR UPDATE`);if(!f.rows.length)throw new Error("Unknown drop");const row=f.rows[0] as any;await assertRareInvariant(tx,row.world_id,id);await tx.execute(sql`DELETE FROM clearing_world_drops WHERE id=${id}`);await tx.execute(sql`UPDATE clearing_reward_chests SET expires_at=LEAST(expires_at,now()) WHERE claimed_at IS NULL AND rewards @> ${JSON.stringify({sourceWorldId:row.world_id,items:[{shopItemId:row.shop_item_id}]})}::jsonb`);return row;});res.json(await getClearingConfig(db,removed.world_id));}catch(e){bad(res,e)}});
