...
export function registerCostumeAdminRoutes(app: Express) {
  ...
  app.put("/api/admin/costume-definitions", requireAdmin, async (req: Request, res: Response) => {
    const body = req.body as { shopItemId?: unknown; templateId?: unknown; placements?: unknown };
    ...
    try {
      const [row] = await db.insert(petCostumeDefinitions).values({
        ...
        set: { placements: parsed.data, updatedAt: new Date() },
      }).returning();
      res.json(row);
    } catch (error) {
