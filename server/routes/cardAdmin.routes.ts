...
import { parseCardLabel } from "../../shared/cardLabel";
...
export function registerCardAdminRoutes(
  ...
  app.get("/api/admin/cards", isAdmin, async (_req, res) => {
    try {
      const result = await db.execute(sql`
        SELECT id, name, description, second_description, artwork_url, effect_color, special_effect, card_label, rarity, created_at, updated_at
  ...
  app.post("/api/admin/cards", isAdmin, async (req, res) => {
    try {
      ...
      const label = parseCardLabel(req.body?.label);
      ...
      const status = /required|characters|fewer|effect color|special effect|card label|hex color/i.test(message) ? 400 : 500;
  ...
  app.patch("/api/admin/cards/:id", isAdmin, async (req, res) => {
    try {
      ...
      const label = hasLabel ? parseCardLabel(req.body?.label) : null;
