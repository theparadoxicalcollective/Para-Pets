...
import { CLEARING_CURRENCY_REWARDS } from "./clearingCurrency";
...
const serialize = (row:any): ClearingRewardChest => ({
  ...
  rewards:row.rewards as ClearingRewardBundle,
});
...
export async function createClearingRewardChest(tx:any,input:{userId:string;sessionId:string;clearingId:string;enemyId:string;petInventoryId:string;exp:number;worldX:number;worldY:number;now?:Date;random?:RandomSource}) {
  const random=input.random??Math.random, now=input.now??new Date();
  const currency=random()<.5?"coins":"essence", range=CLEARING_CURRENCY_REWARDS[currency];
  const amount=range.min+Math.floor(random()*(range.max-range.min+1));
  const equipment:EligibleLoot[]=[];
  if(random()<CLEARING_CHEST_REWARDS.firstEquipmentChance){const item=await rollEquipment(tx,input.clearingId,random);if(item)equipment.push(item);}
  ...
  const rewards:ClearingRewardBundle={exp:input.exp,coins:currency==="coins"?amount:0,essence:currency==="essence"?amount:0,
