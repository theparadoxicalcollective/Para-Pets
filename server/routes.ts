...
import { newAccountPrivileges } from "./registration";
...
<body style="margin:0;padding:0;background-color:#0d0805;font-family:Georgia,serif;">
  ...
  app.delete("/api/admin-messages/:id", isAuthenticated, async (req, res) => {
    try {
      const user = req.user as any;
      const deleted = await deleteOwnedAdminMessage(
        storage.deleteAdminMessageForUsername.bind(storage),
