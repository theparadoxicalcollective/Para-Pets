...
export const CLEARING_POWER_WEIGHTS = { weapon:{atk:2,def:.35,hp:.04}, armor:{atk:.35,def:2,hp:.06}, charm:{atk:1,def:1,hp:.05} } as const;
...
export function auditClearingEquipment(items:Array<any>) {
  ...
  return items.map(item=>{const issues:string[]=[];const stars=Number(item.stars??item.starRarity);const slot=item.slot??item.clearingSlot;
    ...
    if(!["weapon","armor","charm"].includes(slot))issues.push("missing_slot"); if([item.atkBonus,item.defBonus,item.hpBonus].some(v=>Number(v)<0))issues.push("negative_stat");
