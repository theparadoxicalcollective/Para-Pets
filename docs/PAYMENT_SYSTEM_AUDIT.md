...
Original payment write order was: payment row, player coin update, asynchronous community bundle/inbox rows and acquisition badges, egg inventory plus hatch timestamp, asynchronous contribution progress, lifetime purchase query/founder upsert, and watcher chat. Webhook order was materially similar but nested egg after contribution/founder. Milestone *claim* remains a later, separate player action, but now uses its own transaction to lock the same player/cycle/progress boundary, grant the admin-configured coins/item, insert claimed state, and advance the final cycle together.
...
* Milestone rewards remain administrator-configured production data; historical claims do not snapshot the then-current display configuration. See [`PURCHASE_MILESTONE_AUDIT.md`](./PURCHASE_MILESTONE_AUDIT.md).
