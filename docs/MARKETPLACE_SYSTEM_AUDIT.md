...
All marketplace HTTP registrations remain in `server/routes.ts`. Reads are `GET /api/market`, `GET /api/market/my-listings`, `GET /api/market/listing/:listingId/item-details`, and `GET /api/market/listing/:listingId/pet-details`. Mutations are `POST /api/market/list`, `POST /api/market/list-fish`, `POST /api/market/:listingId/buy`, `POST /api/market/:listingId/collect`, `DELETE /api/market/:listingId`, and the separate slot purchase `POST /api/market/buy-slot`. `MarketPage.tsx` is the only marketplace caller: its browse and my-shop queries call the reads; its list, fish-list, buy, collect, cancel, and slot mutations call the corresponding mutations. Pet listing may first call the existing revert-to-egg endpoint.
...
Listing duration/expiry is not represented in the current schema and remains unsupported. Partial-stack quantity listing is not introduced. The existing two-call revert-pet-then-list client flow remains separate because reverting a hatched pet is an explicitly confirmed gameplay action; a listing failure leaves the pet as the requested egg but does not move market value. Slot purchases use their existing conditional single-statement debit/increment path and are outside listing lifecycle escrow/proceeds. Prices, fees (none), slot limits, sorting, eligibility, visuals, assets, and navigation are unchanged.
