...
import { executeAcceptGift, executeSendGift, type GiftTransactionOperations, type SendGiftInput } from "../server/gifts/transactions";
...
class GiftFake implements GiftTransactionOperations {
  ...
  private atomic<T>(work: () => T | Promise<T>): Promise<T> {
    const run = async () => {
      const snapshot = structuredClone({ coins: this.coins, inventory: this.inventory, decor: this.decor, gifts: this.gifts });
      try { return await work(); } catch (error) {
        this.coins = snapshot.coins; this.inventory = snapshot.inventory; this.decor = snapshot.decor; this.gifts = snapshot.gifts;
        throw error;
      }
    };
...
test("gift acceptance succeeds once and retries without duplicating value", async () => {
  ...
  assert.equal(fake.coins.get("receiver"), 20); assert.equal(fake.gifts.get(gift.id).status, "accepted");
});

test("two concurrent acceptance attempts produce only one grant", async () => {
  const fake = new GiftFake(); fake.decor.set("sender:chair", 1);
  const gift = await executeSendGift(baseGift({ itemType: "decor", decorItemId: "chair" }), fake);
  await Promise.all([executeAcceptGift(gift.id, "receiver", fake), executeAcceptGift(gift.id, "receiver", fake)]);
  assert.equal(fake.decor.get("receiver:chair"), 1);
});
