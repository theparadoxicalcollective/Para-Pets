...
test("registered route delegates only to the transactional helper and market remains separate", () => {
  ...
  assert.match(routes, /app\.post\("\/api\/market\/list-fish"/);
  assert.doesNotMatch(routes.slice(routes.indexOf('app.post("/api/market/list-fish"'), routes.indexOf('app.post("/api/fishing/sell"')), /sellFish\(/);
});
