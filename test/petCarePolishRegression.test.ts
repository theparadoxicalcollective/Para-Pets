...
test("Pet Care drags the original shelf artwork above the shelf front", () => {
  const drag = readFileSync("client/src/petCarePolish.ts", "utf8");
  const css = readFileSync("client/src/petCarePolish.css", "utf8");
...
test("Pet Care frames receive subtle shadows and calibrated fills", () => {
  ...
  assert.match(css, /\.pet-care-overlay \.pet-care-meter--mood[\s\S]*?--pet-care-track-top: 34%;[\s\S]*?--pet-care-track-right: 4\.3%;[\s\S]*?--pet-care-track-bottom: 25%/);
  assert.match(css, /\.pet-care-overlay \.pet-care-meter__mood-face-window[\s\S]*?top: 24\.5%;[\s\S]*?left: 6\.7%;[\s\S]*?width: 18\.7%/);
