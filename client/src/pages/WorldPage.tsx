...
export default function WorldPage({ user, onContentReady }: WorldPageProps) {
  ...
  // (navHidden effect is below — after cauldronOpen is declared)
    ...
    >
      ...
      >
          ...
          >
            ...
            <WorldLocations
              ...
              onFlipLocation={(locationId) => flipMutation.mutate(locationId)}
              onDeleteLocation={(loc) => {
                if (confirm(`Delete "${loc.name}"?`)) deleteLocationMutation.mutate(loc.id);
              }}
              onResizeLocation={(locationId, iconSize) => iconSizeMutation.mutate({ locationId, iconSize })}
              onDuplicateLocation={(locationId) => duplicateLocationMutation.mutate(locationId)}
            />
      ...
      {showShop && (
        <WorldShopOverlay
          ...
          accent={accent}
          location={locations.find(location => location.id === activeLocationId)}
          items={items}
          itemsLoading={itemsLoading}
          bundles={shopBundlesForSale}
          decor={shopDecorForSale}
          coins={currentUser.coins}
          isAdmin={currentUser.isAdmin}
          ownedShopItemIds={ownedItemIds}
          ownedPetCount={(itemId) => inventory.filter(item => item.shopItemId === itemId).length}
          showSellPanel={showSellPanel}
          isPurchasePending={buyMutation.isPending}
          isBundlePurchasePending={buyBundleMutation.isPending}
          onPurchase={(itemId, quantity) => buyMutation.mutateAsync({ itemId, quantity })}
          onBuyBundle={(bundleId) => buyBundleMutation.mutate(bundleId)}
          onUnassignItem={(itemId) => activeLocationId && unassignItemMutation.mutate({ locationId: activeLocationId, itemId })}
          onOpenItemPicker={() => { setShowItemPicker(true); refetchAllShopItems(); }}
          onToggleSellPanel={() => { setShowSellPanel(previous => !previous); setSellSelected({}); }}
          onClose={() => { setShowShop(false); setShowItemPicker(false); setShowSellPanel(false); setSellSelected({}); }}
        />
      )}
