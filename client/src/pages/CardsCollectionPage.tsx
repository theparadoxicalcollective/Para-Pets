        ...
        >
          ...
          {visibleCards.map(card => {
            ...
            <button type="button" aria-label={`View ${card.name}`} onClick={() => setSelectedCardId(card.id)} className="block w-full rounded-lg focus-visible:outline focus-visible:outline-amber-200">
              <CardPreview textSize="inventory" rarity={card.rarity} artworkUrl={card.artworkUrl} effectColor={card.effectColor} specialEffect={card.specialEffect} name={card.name} description={card.description} layout={getCardBorderLayout(layouts, card.rarity)} />
            </button>
