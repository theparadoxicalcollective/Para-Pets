...
import iconGlobeWorld    from "@assets/icon_globe_world.webp";
...
import worldEnchanted    from "@assets/bg_enchanted_grove_map.webp";
import worldHaunted      from "@assets/bg_haunted_woods_v2.webp";
import worldDesert       from "@assets/bg_desert_map.webp";
import worldSky          from "@assets/bg_sky_realm_map.webp";
import worldIsland       from "@assets/bg_island_map.webp";
import worldSwamp        from "@assets/bg_swamp_map.webp";
import worldVolcanic     from "@assets/bg_volcanic_map_v4.webp";
import worldSnowy        from "@assets/bg_snowy_mountain_map.webp";
...
function NoticeCarousel({ user }: { user: unknown }) {
  ...
  const notices: ActiveNotice[] = dbNotices.length > 0
    ? dbNotices.map(n => ({ img: n.image_url, href: n.href || null, label: n.label, id: n.id }))
    : NOTICES.map(n => ({ img: n.img, href: (n as any).href ?? null, label: n.label, id: null }));

  const safeIdx = notices.length > 0 ? Math.min(idx, notices.length - 1) : 0;

  const restartTimer = useCallback(() => {
    if (timerRef.current) clearInterval(timerRef.current);
    const len = Math.max(1, notices.length);
    timerRef.current = setInterval(() => setIdx(i => (i + 1) % len), 4200);
  }, [notices.length]);
      ...
      {cur && (
        ...
        >
          <img
            src={cur.img}
            alt={cur.label}
            data-testid={`notice-img-${safeIdx}`}
            style={{
              width: "100%", maxHeight: 468, objectFit: "cover", objectPosition: "top", display: "block",
              border: "1.5px solid rgba(212,168,67,0.3)", borderRadius: 18,
      ...
      <div className="flex justify-center gap-2 mt-3">
        {notices.map((_, i) => (
          <button key={i} onClick={() => goTo(i)} data-testid={`notice-dot-${i}`}
            style={{
              width: i === safeIdx ? 22 : 7, height: 7, borderRadius: 3.5, border: "none", padding: 0,
              background: i === safeIdx ? "#d4a843" : "rgba(212,168,67,0.18)",
              cursor: "pointer", transition: "width 0.32s ease, background 0.32s ease",
            }} />
        ))}
      </div>
    </div>
    ...
    <>
      ...
      >
        ...
        <main className="relative max-w-3xl mx-auto px-5 pb-28" data-testid="hub-main" style={{ zIndex: 1 }}>
          ...
          {/* ── Daily Rewards ──────────────────────────────────────────────── */}
          <div className="mt-4">
            <DailyClaimCard user={user} onSignInRequest={() => setShowSignIn(true)} />
          </div>
