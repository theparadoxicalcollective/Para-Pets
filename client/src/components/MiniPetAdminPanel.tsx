...
export default function MiniPetAdminPanel() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: wasEditing ? "Mini Pet updated" : "Mini Pet added", description: wasEditing ? "Price and details saved." : "Now add and position its animation parts. Closed Eyes and Head Accessory are optional." });
    },
            ...
            <article key={pet.shopItemId} className="rounded-xl p-3" style={{ background: "rgba(0,0,0,.36)", border: "1px solid rgba(251,146,60,.25)" }}>
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[10px] capitalize text-emerald-300/70">{pet.animationStyle} animation · {pet.parts.length}/10 parts</p>
                </div>
