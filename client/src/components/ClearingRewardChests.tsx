  ...
  const modal=<div data-interactive role="presentation" className="fixed inset-0 flex items-center justify-center bg-black/70" style={{zIndex:4000,padding:"max(12px, env(safe-area-inset-top)) max(12px, env(safe-area-inset-right)) max(12px, env(safe-area-inset-bottom)) max(12px, env(safe-area-inset-left))"}} onPointerDown={event=>event.stopPropagation()}>
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="clearing-chest-title" className="relative flex w-[min(calc(100vw-24px),28rem)] max-w-full flex-col overflow-hidden rounded-2xl border-2 border-amber-400/80 bg-emerald-950 p-4 text-amber-50 shadow-2xl" style={{maxHeight:"calc(100dvh - max(24px, env(safe-area-inset-top)) - max(24px, env(safe-area-inset-bottom)))"}}>
      ...
      {detail?<ItemDetails item={detail} onBack={()=>setDetail(null)}/>:<>
        <div data-testid="clearing-chest-reward-list" className={`my-3 grid min-h-0 ${r.items.length===1?"grid-cols-1 justify-items-center":"grid-cols-2 max-[340px]:grid-cols-1"} gap-3 overflow-y-auto overscroll-contain px-1`}>
                    {r.coins>0&&<div className="clearing-reward-tile"><RewardImage src={currencyAssets.coin} alt="Coins" className="h-10 w-10"/><b>{r.coins}</b></div>}
          {r.essence>0&&<div className="clearing-reward-tile"><RewardImage src={currencyAssets.essenceToken} alt="Essence" className="h-10 w-10"/><b>{r.essence}</b></div>}
          {r.items.map((item,index)=><div key={`${item.shopItemId}-${index}`} className="clearing-reward-tile relative min-h-[96px] w-full max-w-[10rem] border-violet-400/50"><RewardImage src={item.imageUrl} alt={item.name} className="h-[clamp(2.75rem,13vw,3.5rem)] w-[clamp(2.75rem,13vw,3.5rem)]"/><b className="text-center text-xs">{item.name}{item.quantity>1?` ×${item.quantity}`:""}</b>{item.starRarity>0&&<span aria-label={`${item.starRarity} star rarity`} className="text-sm text-amber-300">{"★".repeat(item.starRarity)}</span>}<small className="capitalize">{item.type} · {item.rarity}</small></div>)}
        </div>
