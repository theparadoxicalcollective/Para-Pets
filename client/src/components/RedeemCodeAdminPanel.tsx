...
export default function RedeemCodeAdminPanel() {
  ...
  const createCode = useMutation({
    ...
    onSuccess: async () => {
      setCode(""); setName(""); setMessage(""); setCoins(""); setMaxRedemptions(""); setItems([]); setCards([]);
  ...
  const canCreate = code.trim().length >= 3 && name.trim().length > 0 && Number.isSafeInteger(Number(maxRedemptions)) && Number(maxRedemptions) >= 1 && Number(maxRedemptions) <= 1_000_000 && ((Number(coins) || 0) > 0 || items.length > 0 || cards.length > 0);
  ...
  return (
    <div className="space-y-4" data-testid="admin-redeem-code-panel">
      <div className="rounded-xl p-4" style={{ background: "linear-gradient(145deg,rgba(35,16,64,.92),rgba(15,42,31,.94))", border: "1px solid rgba(192,132,252,.38)", boxShadow: "0 8px 24px rgba(0,0,0,.3)" }}>
        ...
        <p className="font-fantasy text-[#a89878] text-[9px] mb-4">Set how many verified accounts may use this code. Each account can use it once, and can use other codes too.</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          ...
          <label className="font-fantasy text-[9px] text-[#a89878]">Number of redemptions<input data-testid="input-admin-code-max-redemptions" type="number" min="1" max="1000000" step="1" value={maxRedemptions} onChange={e => setMaxRedemptions(e.target.value)} placeholder="e.g. 100" className="block w-full mt-1 rounded-md px-3 py-2 font-sans text-xs outline-none" style={inputStyle} /></label>
        </div>
      ...
      <div className="rounded-xl p-4" style={{ background: "rgba(5,20,14,.88)", border: "1px solid rgba(192,132,252,.25)" }}>
        ...
        {isLoading ? <p className="font-fantasy text-[#a89878] text-[10px]">Loading codes…</p> : codes.length === 0 ? <p className="font-fantasy text-[#a89878] text-[10px]">No redeem codes have been created yet.</p> : <div className="space-y-2">{codes.map(entry => {
          ...
          const usedUp = entry.max_redemptions !== null && entry.redemption_count >= entry.max_redemptions;
          return <div key={entry.id} data-testid={`admin-code-${entry.id}`} className="flex items-center gap-3 rounded-lg px-3 py-2" style={{ background: "rgba(0,0,0,.3)", border: `1px solid ${entry.active && !expired && !usedUp ? "rgba(110,231,183,.28)" : "rgba(140,120,110,.2)"}` }}>
            <div className="min-w-0 flex-1"><div className="flex items-center gap-1.5"><span className="font-fantasy text-[#f0c040] text-xs tracking-wider">{entry.code}</span>{entry.active && !expired && !usedUp ? <Check size={11} color="#6ee7b7" /> : null}</div><p className="font-fantasy text-[#b9aa8c] text-[9px] truncate">{entry.bundle_name} · {Number(entry.coin_amount || 0).toLocaleString()} coins · {entry.redemption_count}{entry.max_redemptions === null ? " redeemed (no limit)" : ` / ${entry.max_redemptions} redeemed`}{usedUp ? " · limit reached" : expired ? " · expired" : ""}</p></div>
