...
export default function ElysianClearingCombat({ petPos, petSize, activePet, facingLeft, onRespawn, worldPixels, hudElement, onGameplayBlockedChange }: { petPos:PetWalkPos; petSize:number; activePet?:any; facingLeft:boolean; onRespawn:()=>void; worldPixels:WorldPixels; hudElement:HTMLElement|null; onGameplayBlockedChange:(blocked:boolean)=>void }) {
  ...
  const ground=useClearingGroundDrops(session?.sessionId??null); const equipment=useClearingEquipment();const currency=useClearingCurrencyDrops(session?.sessionId??null);const {data:user}=useQuery<any>({queryKey:["/api/user"]});
  ...
  const fixedHud = <>
    <div className="absolute pointer-events-auto flex gap-2" style={{left:14,top:"max(58px, calc(env(safe-area-inset-top, 0px) + 58px))",zIndex:18}}><button data-interactive data-testid="button-clearing-equipment" aria-label="Open pet equipment" onClick={()=>setEquipmentOpen(true)} className="h-12 w-12 overflow-hidden rounded-full border-2 border-amber-300 bg-emerald-950 shadow-lg"><img src={activePet?.isHatched?(activePet.hatchedImageUrl||activePet.imageUrl||fallbackPet):(activePet?.eggImageUrl||fallbackPet)} onError={e=>{e.currentTarget.src=fallbackPet}} alt="Active pet" className="h-full w-full object-contain"/></button><button data-interactive aria-label="Open Clearing inventory" onClick={()=>setInventoryOpen(true)} className="h-12 w-12 rounded-full border-2 border-amber-300 bg-emerald-950 p-1 shadow-lg"><img src={bagIcon} alt="Inventory" className="h-full w-full object-contain"/></button></div>
    <div data-testid="clearing-currency-display" className="absolute right-3 pointer-events-none flex gap-2 rounded-xl border border-amber-500/70 bg-emerald-950/90 px-2 py-1 text-xs font-bold text-amber-100 shadow-lg" style={{top:"max(58px, calc(env(safe-area-inset-top, 0px) + 58px))",zIndex:17}}><span className="flex items-center gap-1"><img src={coinIcon} alt="Coins" className="h-5 w-5 object-contain"/>{Number(user?.coins??0).toLocaleString()}</span><span className="flex items-center gap-1"><img src={essenceIcon} alt="Essence" className="h-5 w-7 object-contain"/>{Number(user?.essence??0).toLocaleString()}</span></div>
