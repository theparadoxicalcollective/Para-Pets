...
type Session = { sessionId:string; loadout:ClearingLoadout; pet:{inventoryId:string;maxHealth:number;attack:number}; enemies:Array<Omit<Enemy,"x"|"y"|"targetX"|"targetY"|"state"|"facingLeft"|"nextActionAt">>; chests:ClearingRewardChest[] };
...
export default function ElysianClearingCombat({ petPos, petSize, activePet, facingLeft, onRespawn, worldPixels, hudElement, onGameplayBlockedChange }: { petPos:PetWalkPos; petSize:number; activePet?:any; facingLeft:boolean; onRespawn:()=>void; worldPixels:WorldPixels; hudElement:HTMLElement|null; onGameplayBlockedChange:(blocked:boolean)=>void }) {
  ...
  const [chests,setChests]=useState<ClearingRewardChest[]>([]);const [selectedChest,setSelectedChest]=useState<ClearingRewardChest|null>(null);const [claimingChest,setClaimingChest]=useState(false);const [chestError,setChestError]=useState<string|null>(null);
  const [equipmentOpen,setEquipmentOpen]=useState(false);const [inventoryOpen,setInventoryOpen]=useState(false);
  const gameplayState: "loading"|"running"|"menu-paused"|"defeated" = sessionState!=="ready" ? "loading" : equipmentOpen||inventoryOpen||Boolean(selectedChest) ? "menu-paused" : petHealth<=0 ? "defeated" : "running";
  const gameplayStateRef=useRef(gameplayState);gameplayStateRef.current=gameplayState;
  const pausedAt=useRef<number|null>(null);

  useEffect(() => {
    onGameplayBlockedChange(equipmentOpen||inventoryOpen||Boolean(selectedChest));
    return () => onGameplayBlockedChange(false);
  }, [equipmentOpen,inventoryOpen,selectedChest, onGameplayBlockedChange]);
  ...
  return <>
    <ClearingChestLayer chests={chests} onOpen={chest=>{setChestError(null);setSelectedChest(chest)}}/>
