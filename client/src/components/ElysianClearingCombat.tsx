...
export default function ElysianClearingCombat({ petPos, petSize, activePet, facingLeft, onRespawn, worldPixels, hudElement, onGameplayBlockedChange }: { petPos:PetWalkPos; petSize:number; activePet?:any; facingLeft:boolean; onRespawn:()=>void; worldPixels:WorldPixels; hudElement:HTMLElement|null; onGameplayBlockedChange:(blocked:boolean)=>void }) {
  ...
  const attack=useCallback(async(e:React.PointerEvent)=>{e.preventDefault();e.stopPropagation();const now=performance.now();if(gameplayStateRef.current!=="running"||!session){setFeedback([sessionState==="loading"?"Combat loading…":"Combat paused"]);return;}if(now<defeatedUntil.current)return;if(now-lastAttack.current<CFG.petAttackCooldownMs){setFeedback(["Cooling down"]);return;}lastAttack.current=now;setSwinging(true);setTimeout(()=>setSwinging(false),220);
