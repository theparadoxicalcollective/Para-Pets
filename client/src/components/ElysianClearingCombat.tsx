...
export default function ElysianClearingCombat({ petPos, facingLeft, onRespawn, worldPixels }: { petPos:PetWalkPos; facingLeft:boolean; onRespawn:()=>void; worldPixels:WorldPixels }) {
  ...
  useEffect(()=>{let raf=0,last=performance.now(),lastPaint=0;
    const tick=(now:number)=>{const dt=Math.min((now-last)/1000,.05);last=now; const pet=petPosRef.current;
      for(const e of enemiesRef.current){
        ...
        else if(e.state==="windup"&&now>=e.nextActionAt){if(d<=CFG.attackRangePixels&&now>=invulnerableUntil.current&&defeatedUntil.current<=now){const hp=Math.max(0,petHealthRef.current-e.attack);petHealthRef.current=hp;setPetHealth(hp);invulnerableUntil.current=now+CFG.petInvulnerabilityMs;if(hp===0){defeatedUntil.current=now+1400;invulnerableUntil.current=defeatedUntil.current+CFG.respawnProtectionMs;const timer=setTimeout(()=>{onRespawn();const full=petMaxHealthRef.current;petHealthRef.current=full;setPetHealth(full);timers.current.delete(timer);},1400);timers.current.add(timer);}}e.state="recovering";e.nextActionAt=now+CFG.attackCooldownMs;}
