...
.evo-wrap{max-width:560px;margin:8px auto 12px;padding:12px 12px 13px;border:1px solid #387f64;border-radius:18px;background:linear-gradient(180deg,#071c18eb,#04110fee);box-shadow:inset 0 0 24px #19c87914,0 8px 20px #0006;box-sizing:border-box;width:100%}
.evo-head{display:flex;align-items:flex-end;justify-content:space-between;gap:10px;margin-bottom:8px}.evo-title{font:700 12px/1 system-ui,sans-serif;letter-spacing:.18em;color:#baffda}.evo-copy{font:600 11px/1.2 system-ui,sans-serif;color:#8bbba5;text-align:right}.evo-slots{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:5px}.evo-slot{position:relative;aspect-ratio:1;border:0;background:transparent;padding:0;min-width:0;cursor:default;-webkit-tap-highlight-color:transparent}.evo-slot.current{cursor:pointer}.evo-slot img{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;pointer-events:none}.evo-slot .evo-base{filter:grayscale(1) brightness(.48);opacity:.72}.evo-slot.locked{opacity:.58;filter:grayscale(.75)}.evo-slot.current{filter:drop-shadow(0 0 8px #4dffac66)}.evo-slot.complete{filter:drop-shadow(0 0 7px #2cff9966)}.evo-fill{position:absolute;inset:0;overflow:hidden}.evo-fill img{filter:saturate(1.08) brightness(1.05)}.evo-lock-badge{position:absolute;right:2%;bottom:1%;width:31%;aspect-ratio:1;border-radius:50%;display:grid;place-items:center;background:#07120fdc;border:1px solid #8e9c95;color:#cad2ce;box-shadow:0 2px 5px #0008}.evo-current-bar{margin-top:9px}.evo-bar-label{display:flex;justify-content:space-between;gap:8px;margin-bottom:4px;font:700 11px/1 system-ui,sans-serif;color:#b7e9d0}.evo-track{height:9px;border-radius:999px;border:1px solid #3f735f;background:#03100c;overflow:hidden}.evo-track>i{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,#147d52,#36e890,#91ffd0);box-shadow:0 0 9px #3effa177;transition:width .35s ease}.evo-note{margin-top:6px;font:500 10px/1.3 system-ui,sans-serif;color:#769d8b;text-align:center}.evo-picker-backdrop{position:fixed;inset:0;z-index:10020;background:#000b;display:flex;align-items:flex-end;justify-content:center;padding:14px;box-sizing:border-box}.evo-picker{position:relative;width:min(100%,560px);max-height:min(78vh,720px);overflow:hidden;border:1px solid #55b987;border-radius:22px;background:linear-gradient(180deg,#0b211b,#06110f);box-shadow:0 -12px 55px #000;display:flex;flex-direction:column}.evo-picker-head{padding:16px 50px 10px 16px;border-bottom:1px solid #22533f}.evo-picker-head h3{margin:0;color:#c8ffe1;font:700 20px/1.1 Georgia,serif}.evo-picker-head p{margin:5px 0 0;color:#9fc5b2;font:500 12px/1.35 system-ui,sans-serif}.evo-picker-close{position:absolute;right:12px;top:12px;width:36px;height:36px;border-radius:50%;border:1px solid #478267;background:#071711;color:#d7ffea;display:grid;place-items:center}.evo-feeders{overflow-y:auto;padding:10px 12px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;overscroll-behavior:contain}.evo-feeder{position:relative;display:grid;grid-template-columns:58px 1fr;align-items:center;gap:8px;min-width:0;padding:8px;border:1px solid #2f604c;border-radius:13px;background:#081812;color:#dfffee;text-align:left}.evo-feeder.selected{border-color:#68f0ad;background:#0c2a1d;box-shadow:inset 0 0 14px #29d88122}.evo-feeder img{width:58px;height:58px;object-fit:contain}.evo-feeder-name{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font:700 12px/1.2 system-ui,sans-serif}.evo-feeder-meta{display:block;margin-top:3px;color:#9ec9b4;font:600 10px/1.2 system-ui,sans-serif}.evo-check{position:absolute;right:6px;top:6px;width:18px;height:18px;border-radius:50%;border:1px solid #5a8d73;background:#06100d;color:#062012;font:900 12px/17px system-ui;text-align:center}.selected .evo-check{background:#6ff0ae;border-color:#adffd3}.evo-picker-foot{padding:10px 12px 12px;border-top:1px solid #22533f;background:#06110f}.evo-total{display:flex;justify-content:space-between;gap:10px;margin-bottom:8px;color:#c8ffe1;font:700 12px/1.2 system-ui,sans-serif}.evo-warning{margin:0 0 8px;color:#e5b9a8;font:600 10px/1.3 system-ui,sans-serif}.evo-feed-btn{width:100%;min-height:44px;border-radius:999px;border:1px solid #67e8a9;background:linear-gradient(#147a51,#0b5538);color:#eafff3;font:800 13px/1 system-ui,sans-serif;letter-spacing:.04em}.evo-feed-btn:disabled{opacity:.45}.evo-error{padding:8px 12px;color:#ffb4a8;font:600 11px/1.35 system-ui,sans-serif}.evo-empty{grid-column:1/-1;padding:25px 10px;text-align:center;color:#a6c9b8;font:600 12px/1.4 system-ui,sans-serif}
@media(max-width:430px){.evo-wrap{border-radius:15px;padding:10px 8px}.evo-slots{gap:2px}.evo-feeders{grid-template-columns:1fr}.evo-picker-backdrop{padding:8px}}
`;
...
export default function EvolutionPanel({ enabled, fallbackRarity }: EvolutionPanelProps) {
  ...
  return <>
    ...
    <section className="evo-wrap" data-testid="section-pet-evolution">
      ...
      <div className="evo-slots" aria-label="Evolution slots">
        {Array.from({ length: slotCount }, (_, index) => {
          ...
          return <button
            ...
            disabled={!current}
            aria-label={complete ? `Evolution slot ${index + 1} complete` : current ? `Evolution slot ${index + 1}, ${Math.round(fill)} percent filled` : `Evolution slot ${index + 1} locked`}
          ...
          >
            ...
            {locked && <span className="evo-lock-badge"><Lock size={11} /></span>}
          </button>;
        })}
      </div>
      ...
      <div className="evo-note">
        {loading ? "Reading evolution energy…" : state?.isComplete ? "All evolution icons are filled." : "Tap the unlocked icon to choose feeder pets. Icons unlock one at a time."}
      </div>
    ...
    {pickerOpen && <div className="evo-picker-backdrop" role="presentation" onPointerDown={(event) => { if (event.target === event.currentTarget) setPickerOpen(false); }}>
      <div className="evo-picker" role="dialog" aria-modal="true" aria-label="Choose evolution feeder pets">
        ...
        <div className="evo-picker-head">
          ...
          <p>Select pets to convert into evolution points. Higher-rarity pets provide more points.</p>
        </div>
