  ...
  const picker = pickerOpen && typeof document !== "undefined" ? createPortal(
    <div className="pupevo-picker-backdrop" role="presentation" onPointerDown={(event) => { if (event.target === event.currentTarget) setPickerOpen(false); }}>
      <div className="pupevo-picker" role="dialog" aria-modal="true" aria-label="Choose evolution feeder pets">
        ...
        <div className="pupevo-picker-foot">
          <div className="pupevo-total"><span>{selectedPets.length} pet{selectedPets.length === 1 ? "" : "s"} selected</span><span>+{selectedPoints} pts</span></div>
          <p className="pupevo-warning">Feeder pets are permanently consumed when you confirm. This action cannot be undone.</p>
      ...
      {Array.from({ length: slotCount }, (_, index) => {
        ...
        return <button
          ...
          onClick={() => {
            ...
            else showNodeMessage(claimed ? "Reward Collected" : complete ? "Completed" : "Locked");
          }}
