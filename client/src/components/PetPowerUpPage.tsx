...
@media(max-width:430px){.pupage-head{min-height:92px;padding:0 42px}.pupage-logo{width:86vw}.pupage-corner{width:34px;height:34px}.pupage-capacity{width:90%;padding:5px 10px;margin-top:0}.pupage-cap-track{height:10px}.pupage-enhance{width:52%}.pupage-stage{height:clamp(325px,89vw,380px);margin-top:-6px}.pupage-pet-zone{width:min(62vw,260px);height:min(62vw,260px);top:37%}.pupage-platform{width:76%;top:51%}.pupage-nameplate{width:70%}.pupage-inventory{margin-top:-27px}.pupage-stat-row{left:22%;right:9%;grid-template-columns:minmax(0,1fr) 38px 34px;gap:2px}.pupage-items-window{left:13%;right:13%;top:24%;height:49%}.pupage-arrow{width:32px;height:41px}.pupage-stats-close{width:32px;height:32px}}
@media(max-height:720px){.pupage-scroll{padding-top:max(env(safe-area-inset-top),4px);padding-bottom:max(env(safe-area-inset-bottom),5px)}.pupage-head{min-height:78px}.pupage-logo{width:min(78vw,470px)}.pupage-capacity{padding-top:4px;padding-bottom:4px}.pupage-enhance{width:min(48%,255px)}.pupage-stage{height:clamp(270px,78vw,330px);margin-top:-5px}.pupage-pet-zone{width:min(56vw,235px);height:min(56vw,235px)}.pupage-nameplate{width:min(70%,375px)}.pupage-inventory{width:94%;margin-top:-23px}.pupage-bottom-spacer{height:2px}}
@media(max-height:640px){.pupage-head{min-height:68px}.pupage-stage{height:clamp(245px,70vw,295px)}.pupage-inventory{width:88%;margin-top:-19px}.pupage-enhance{width:min(46%,235px)}}
`;
...
export default function PetPowerUpPage(props: PetUpgradeModalProps) {
  ...
  return <div className="pupage" style={{ zIndex: z }} role="dialog" aria-modal="true" aria-label="Power Up">
    ...
    {statsOpen && <div className="pupage-stats-backdrop" onClick={closeStats}><div className="pupage-stats-modal" role="dialog" aria-modal="true" aria-label={`${petName} stats`} onClick={(event) => event.stopPropagation()}><button ref={statsCloseRef} type="button" className="pupage-stats-close" onClick={closeStats} aria-label="Close pet stats"><img src={pupCloseButton} alt="" /></button><section className="pupage-stats" data-testid="section-pet-stats"><img src={pupStatBox} alt="Pet stats" className="pupage-stat-art" />{stats.map((stat) => <div key={stat.key} className={`pupage-stat-row ${stat.key}`}><span className="pupage-stat-label" aria-hidden="true">{stat.label}</span><span className="pupage-sr">{stat.label}</span><div className="pupage-stat-track" data-testid={`bar-stat-${stat.key}`}><div className="pupage-stat-fill" style={{ width: `${Math.min(100, Math.max(3, stat.value / stat.max * 100))}%` }} /></div><span className="pupage-stat-value" data-testid={`text-stat-${stat.key}`}>{stat.value}</span><button type="button" className="pupage-stat-plus" onClick={() => openItems(stat.filter)} aria-label={`Show Power Up items for ${stat.label}`}>+</button></div>)}</section></div></div>}
