...
export function useClearingEquipment(){
  ...
  const equip=useMutation({mutationFn:async(inventoryId:string)=>(await apiRequest("POST","/api/clearing/loadout/equip",{inventoryId})).json() as Promise<ClearingLoadoutResponse>,onSuccess:async data=>{queryClient.setQueryData(loadoutKey,data);await queryClient.invalidateQueries({queryKey:inventoryKey});}});
  const unequip=useMutation({mutationFn:async(slot:ClearingEquipmentSlot)=>(await apiRequest("POST","/api/clearing/loadout/unequip",{slot})).json() as Promise<ClearingLoadoutResponse>,onSuccess:async data=>{queryClient.setQueryData(loadoutKey,data);await queryClient.invalidateQueries({queryKey:inventoryKey});}});
