...
/** Soft positional separation. A small overlap is deliberately accepted so
 * tightly grouped sprites do not alternate across the collision boundary on
 * successive frames. The capped fraction also leaves steering velocity alone. */
export function resolveEnemyPairs(enemies:SimEnemy[],world:{width:number;height:number}){let pairs=0;const deadZone=2,maximumStep=1.5;for(let i=0;i<enemies.length;i++)for(let j=i+1;j<enemies.length;j++){pairs++;const a=enemies[i],b=enemies[j],rawX=(b.x-a.x)*world.width,rawY=(b.y-a.y)*world.height,d=Math.hypot(rawX,rawY),dx=d?rawX:((i+j)%2?1:-1),dy=d?rawY:0,min=enemyMinimumSeparation(a,b),overlap=min-d-deadZone;if(overlap>0){const correction=Math.min(maximumStep,overlap*.18);a.x-=dx/Math.max(1,d)*correction/world.width;a.y-=dy/Math.max(1,d)*correction/world.height;b.x+=dx/Math.max(1,d)*correction/world.width;b.y+=dy/Math.max(1,d)*correction/world.height;}}return pairs;}
