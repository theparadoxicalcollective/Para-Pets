...
export function initializeClearingEnemies<T extends ClearingSessionEnemy>(source: T[], fallbackHomes: readonly { x:number; y:number }[], now: number, delays: readonly number[], visibleHeight: (boss:boolean)=>number) {
  return source.map((enemy, slot) => {
    ...
    // Regular enemies stay passive until struck. Bosses enter the same combat
    // state machine already engaged so they begin pressuring the player shortly
    // after their spawn animation instead of waiting to be attacked first.
    return { ...enemy, engagedByPlayer:Boolean(enemy.isBoss), slot, x:home.x, y:home.y, homeX:home.x, homeY:home.y, targetX:home.x, targetY:home.y, velocityX:0, velocityY:0, visibleHalfWidth:visibleHeight(enemy.isBoss)*.4, state:"spawning" as const, facingLeft:false, nextActionAt:now+(delays[slot]??500) };
  });
}
