...
export function initializeClearingEnemies<T extends ClearingSessionEnemy>(source: T[], fallbackHomes: readonly { x:number; y:number }[], now: number, delays: readonly number[], visibleHeight: (boss:boolean)=>number) {
  return source.map((enemy, slot) => {
    ...
    return { ...enemy, engagedByPlayer:false, slot, x:home.x, y:home.y, homeX:home.x, homeY:home.y, targetX:home.x, targetY:home.y, visibleHalfWidth:visibleHeight(enemy.isBoss)*.4, state:"spawning" as const, facingLeft:false, nextActionAt:now+(delays[slot]??500) };
  });
}
