...
export function selectFirstEnemyAlongAimCapsule<T>(origin:PetWalkPos,direction:ClearingDirection,candidates:ClearingTargetCandidate<T>[],range:number,capsuleRadius:number,world:WorldPixels):T|undefined{return candidates.filter(c=>c.active&&c.health>0).map(candidate=>{const radius=Math.max(0,candidate.collisionRadius),ray=clearingDistanceToRay(origin,direction,candidate.center,range+radius,world),edgeDistance=hitboxEdgeDistance(origin,candidate.center,radius,world);return{candidate,ray,edgeDistance};}).filter(({candidate,ray,edgeDistance})=>ray&&ray.along>=0&&edgeDistance<=range&&ray.distance<=capsuleRadius+Math.max(0,candidate.collisionRadius)).sort((a,b)=>(a.ray?.along??Infinity)-(b.ray?.along??Infinity)||a.edgeDistance-b.edgeDistance)[0]?.candidate.enemy;}
