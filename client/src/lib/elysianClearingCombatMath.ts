...
import { CLEARING_AIM_GEOMETRY, CLEARING_PET_COMBAT_RADIUS, clearingCollisionGapDistance, clearingDistanceToRay, clearingHitboxEdgeDistance, clearingPointInDirection, normalizeClearingDirection, type ClearingDirection } from "@shared/clearingCombatGeometry";
...
export function selectEnemiesAlongAimCapsule<T>(origin:PetWalkPos,direction:ClearingDirection,candidates:ClearingTargetCandidate<T>[],range:number,capsuleRadius:number,world:WorldPixels,limit=2):T[]{return candidates.filter(c=>c.active&&c.health>0).map(candidate=>{const radius=Math.max(0,candidate.collisionRadius),ray=clearingDistanceToRay(origin,direction,candidate.center,range+radius,world),edgeDistance=hitboxEdgeDistance(origin,candidate.center,radius,world);return{candidate,ray,edgeDistance};}).filter(({candidate,ray,edgeDistance})=>ray&&ray.along>=0&&edgeDistance<=range&&ray.distance<=capsuleRadius+Math.max(0,candidate.collisionRadius)).sort((a,b)=>(a.ray?.along??Infinity)-(b.ray?.along??Infinity)||a.edgeDistance-b.edgeDistance).slice(0,Math.max(1,limit)).map(result=>result.candidate.enemy);}
export function selectFirstEnemyAlongAimCapsule<T>(origin:PetWalkPos,direction:ClearingDirection,candidates:ClearingTargetCandidate<T>[],range:number,capsuleRadius:number,world:WorldPixels):T|undefined{return selectEnemiesAlongAimCapsule(origin,direction,candidates,range,capsuleRadius,world,1)[0];}
...
export function resolveMeleeTarget<T extends {instanceId:string}>(origin:PetWalkPos,direction:ClearingDirection,candidates:ClearingTargetCandidate<T>[],world:WorldPixels,lockedInstanceId:string|null=null):ClearingTargetQualification<T>|undefined {
  ...
  const scored=candidates.filter(candidate=>candidate.active&&candidate.health>0).map(candidate=>{const delta=pixelDelta(origin,candidate.center,world),centerDistance=Math.hypot(delta.dx,delta.dy),edgeDistance=clearingCollisionGapDistance(origin,candidate.center,CLEARING_PET_COMBAT_RADIUS,candidate.collisionRadius,world),targetDirection=normalizeDirection(delta)??aim,alignment=centerDistance?(delta.dx*aim.dx+delta.dy*aim.dy)/centerDistance:1;return{enemy:candidate.enemy,centerDistance,edgeDistance,direction:targetDirection,alignment,directlyInStrikeRange:edgeDistance<=CLEARING_AIM_GEOMETRY.meleeAttackRangePixels+(candidate.enemy.instanceId===lockedInstanceId?CLEARING_AIM_GEOMETRY.meleeLockHysteresisPixels:0),onlyWithinAssistRange:edgeDistance>CLEARING_AIM_GEOMETRY.meleeAttackRangePixels};}).filter(result=>result.edgeDistance<=(result.enemy.instanceId===lockedInstanceId?CLEARING_AIM_GEOMETRY.meleeDisengageRangePixels:CLEARING_AIM_GEOMETRY.meleeTargetAssistRadiusPixels));
  const strikeable=scored.filter(result=>result.directlyInStrikeRange),locked=lockedInstanceId?strikeable.find(result=>result.enemy.instanceId===lockedInstanceId):undefined;if(locked)return locked;
  const cone=Math.cos(CLEARING_AIM_GEOMETRY.meleePreferredConeDegrees*Math.PI/360),compare=(a:ClearingTargetQualification<T>,b:ClearingTargetQualification<T>)=>a.edgeDistance-b.edgeDistance||b.alignment-a.alignment||a.enemy.instanceId.localeCompare(b.enemy.instanceId);
...
/** Resolve native art direction by immutable item identity, with a safe style fallback. */
export function weaponArtOffsetForWeapon(input:{stableKey?:string|null;shopItemId?:string|null;attackStyle:"sword_slash"|"staff_orb"|"default_melee"}){
  if(input.stableKey==="clearing-training-sword"||input.shopItemId==="a1b2c3d4-0011-4000-8000-000000000012")return 90;
  if(input.stableKey==="clearing-item:a1b2c3d4-0011-4000-8000-000000000021"||input.shopItemId==="a1b2c3d4-0011-4000-8000-000000000021")return -135;
  return weaponArtOffsetForStyle(input.attackStyle);
}
